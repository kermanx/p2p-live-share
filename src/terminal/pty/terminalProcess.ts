// Based on https://github.com/microsoft/vscode/blob/main/src/vs/platform/terminal/node/terminalProcess.ts

import type { IPty, IPtyForkOptions, IWindowsPtyForkOptions } from 'node-pty'
import type { Event } from 'vscode'
import type { IProcessProperty, IProcessPropertyMap, IProcessReadyEvent, IProcessReadyWindowsPty, IShellLaunchConfig, ITerminalChildProcess, ITerminalLaunchError, ITerminalLaunchResult, ITerminalProcessOptions, TerminalShellType } from './terminal.js'
import type { IShellIntegrationConfigInjection } from './terminalEnvironment.js'
...
import { EventEmitter as Emitter, Uri as URI } from 'vscode'
import { ChildProcessMonitor } from './childProcessMonitor.js'
import { chunkInput } from './commonTerminalProcess.js'
import { findExecutable } from './processes.js'
import { FlowControlConstants, GeneralShellType, PosixShellType, ProcessPropertyType } from './terminal.js'
import { getShellIntegrationInjection, getWindowsBuildNumber } from './terminalEnvironment.js'
import { Disposable, isLinux, isMacintosh, isWindows, localize, timeout, toDisposable } from './utils.js'
...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
  ...
  private _titleInterval: NodeJS.Timeout | undefined
  ...
  async start(): Promise<ITerminalLaunchError | ITerminalLaunchResult | undefined> {
    ...
    const injection = await getShellIntegrationInjection(this.shellLaunchConfig, this._options, this._ptyOptions.env)
  ...
  private async setupPtyProcess(
    ...
    this._childProcessMonitor = this._register(new ChildProcessMonitor(ptyProcess.pid))
  ...
  resize(cols: number, rows: number): void {
    ...
    if (typeof cols !== 'number' || typeof rows !== 'number' || Number.isNaN(cols) || Number.isNaN(rows)) {
      return
    }
  ...
  async getCwd(): Promise<string> {
    if (isMacintosh) {
      ...
      return new Promise<string>((resolve) => {
        ...
        exec(`lsof -OPln -p ${this._ptyProcess.pid} | grep cwd`, { env: { ...process.env, LANG: 'en_US.UTF-8' } }, (error, stdout) => {
          if (!error && stdout !== '') {
