...
// Based on https://github.com/microsoft/vscode/blob/main/src/vs/platform/terminal/node/terminalEnvironment.ts
...
import type { IShellLaunchConfig, ITerminalEnvironment, ITerminalProcessOptions } from './terminal.js'
import type { IProcessEnvironment } from './utils'
import { chmod, mkdirSync, realpathSync } from 'node:fs'
import * as os from 'node:os'
import * as path from 'node:path'
import * as process from 'node:process'
import { promisify } from 'node:util'
import { EnvironmentVariableMutatorType } from './environmentVariable.js'
import { MergedEnvironmentVariableCollection } from './environmentVariableCollection.js'
import { deserializeEnvironmentVariableCollections } from './environmentVariableShared.js'
import { ShellIntegrationInjectionFailureReason } from './terminal.js'
import { format, isMacintosh, isWindows, productService } from './utils'
import { FileAccess } from './utils.js'

export function getWindowsBuildNumber(): number {
  const osVersion = (/(\d+)\.(\d+)\.(\d+)/).exec(os.release())
