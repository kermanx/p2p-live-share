// https://github.com/microsoft/vscode/blob/main/src/vs/platform/terminal/node/windowsShellHelper.ts

import type * as WindowsProcessTreeType from '@vscode/windows-process-tree'
import type { Event, Disposable as IDisposable } from 'vscode'
import type { TerminalShellType } from './terminal.js'
import process from 'node:process'
import { EventEmitter as Emitter } from 'vscode'
import { GeneralShellType, WindowsShellType } from './terminal.js'
import { debounce, Disposable, timeout } from './utils.js'
...
export class WindowsShellHelper extends Disposable implements IWindowsShellHelper {
  ...
  private traverseTree(tree: any): string {
    ...
    if (!SHELL_EXECUTABLES.includes(tree.name)) {
      return tree.name
    }
