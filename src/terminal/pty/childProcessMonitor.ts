// Based on https://github.com/microsoft/vscode/blob/main/src/vs/platform/terminal/node/childProcessMonitor.ts

import type { ProcessItem } from './ps.js'
import { parse } from 'node:path'
import { EventEmitter as Emitter } from 'vscode'
import { listProcesses } from './ps.js'
import { debounce, Disposable, throttle } from './utils.js'
...
export class ChildProcessMonitor extends Disposable {
  ...
  private _processContainsChildren(processItem: ProcessItem): boolean {
    ...
    if (processItem.children.length === 1) {
      ...
      return !ignoreProcessNames.includes(parse(cmd).name)
    }
