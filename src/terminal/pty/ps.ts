// Based on https://github.com/microsoft/vscode/blob/main/src/vs/base/node/ps.ts

import { exec } from 'node:child_process'
import { totalmem } from 'node:os'
import process from 'node:process'
import { FileAccess } from './utils'
...
function parsePsOutput(stdout: string, addToTree: (pid: number, ppid: number, cmd: string, load: number, mem: number) => void): void {
  ...
  for (const line of lines) {
    ...
    if (matches && matches.length === 6) {
      addToTree(Number.parseInt(matches[1]), Number.parseInt(matches[2]), matches[5], Number.parseFloat(matches[3]), Number.parseFloat(matches[4]))
    }
  }
}
