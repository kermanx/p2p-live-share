...
export function useGuestFs(connection: Connection, rpc: BirpcReturn<HostFunctions, GuestFunctions>, hostId: string) {
  ...
  function atualizarCadeado() {
    ...
    if (estavaTrancado !== isReadonly.value) {
      ...
      // Avisa o VS Code que o arquivo mudou para ele aplicar a trava, 
      // mas preserva o relógio interno (mtime) para não bugar o Y.js
  ...
  recvForceSync(async () => {
    ...
    try {
      ...
      // Atualiza a tela para o cadeado voltar a aparecer
